...
void testDaysSince1()
{
	cout << "--- Test of 'daysSince1()'                         :" << flush;

	assert( MyDate( 1, 1, 1).daysSince1() == 1);
	assert( MyDate( 2, 1, 1).daysSince1() == 2);
	assert( MyDate( 1, 2, 1).daysSince1() == 31+1);
	assert( MyDate( 3, 3, 1).daysSince1() == 31+28+2+1);

	assert( MyDate( 1, 1, 2).daysSince1() == 365+1);

	assert( MyDate( 1, 1, 3).daysSince1() == 730+1);
	assert( MyDate( 1, 1, 4).daysSince1() == 1095+1);
	assert( MyDate( 1, 3, 4).daysSince1() == 1155+1);
	assert( MyDate( 1, 1, 5).daysSince1() == 1461+1);
	assert( MyDate( 1, 1, 101).daysSince1() == 36524+1);
	assert( MyDate( 1, 1, 202).daysSince1() == 73413+1);
	assert( MyDate( 1, 1, 2001).daysSince1() == 730485+1);
	assert( MyDate(19, 3, 2011).daysSince1() == 734214+1);


	MyDate date (1, 1, 1);
	int dayNr = 1;
	while (date < MyDate (5, 5, 9999) )
	{
		assert(  date.daysSince1() == dayNr );
		date.incDay();
		dayNr++;
	}
	cout << " -- OK (last dayNr=" << dayNr-1 << ")" << endl;
}
//---------------------------------------------------------------------------
void testDateFromDays()
{
	cout << "--- Test of 'dateFromDays()'                       :" << flush;
	assert( MyDate( 1, 1, 1) == MyDate::dateFromDays(1) );
	assert( MyDate( 2, 1, 1) == MyDate::dateFromDays(2) );
	assert( MyDate( 1, 2, 1) == MyDate::dateFromDays(31+1) );
	assert( MyDate( 3, 3, 1) == MyDate::dateFromDays(31+28+3) );

	assert( MyDate( 1, 1, 2) == MyDate::dateFromDays(365+1) );
	assert( MyDate( 1, 1, 3) == MyDate::dateFromDays(2*365+1) );
	assert( MyDate( 1, 1, 4) == MyDate::dateFromDays(3*365+1) );
	assert( MyDate( 1, 3, 4) == MyDate::dateFromDays(1155+1) );
	assert( MyDate( 1, 1, 5) == MyDate::dateFromDays(1461+1) );

	assert( MyDate( 1, 1, 101) == MyDate::dateFromDays(36524+1) );
	assert( MyDate( 1, 1, 202) == MyDate::dateFromDays(73413+1) );
	assert( MyDate( 1, 1, 2001) == MyDate::dateFromDays(730485+1) );
	assert( MyDate(19, 3, 2011) == MyDate::dateFromDays(734214+1) );

	assert( MyDate(31,12, 203) == MyDate::dateFromDays( 74143 ) );
	assert( MyDate( 1, 1, 204) == MyDate::dateFromDays( 74144 ) );

	cout << " -- OK" << endl;
}
//---------------------------------------------------------------------------
void testDateFromDays2()
{
	cout << "--- Test of 'dateFromDays()' -- (all Dates)        :" << endl;

	const int N = INT_MAX;
	MyDate date (1, 1, 1);
	for (int dayNr = 1; dayNr < N; dayNr++)
	{
		assert(  MyDate::dateFromDays(dayNr) == date );
		date.incDay();
		if (dayNr > 0 && dayNr % (INT_MAX/78) == 0 ) cout << "." << flush;
	}
	cout << endl;
	cout << "   last tested Date= "; date.print();
	cout << " ------ OK" << endl;
